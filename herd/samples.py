...
# The durations of maternal antibodies, which are the same for each SAT.
_filename_ma = os.path.join(_path, 'AbDuration_PosteriorSamples.txt')
_samples_ma = pandas.read_csv(_filename_ma,
                              delim_whitespace=True,
                              header=None)
_samples_ma.index.name = samples.index.name
_samples_ma.columns = pandas.Index(['maternal_immunity_duration_shape',
                                    'maternal_immunity_duration_mean'],
                                   name='parameter')
# Add the maternal-antibody parameters to each SAT.
for _param in _samples_ma.columns:
    for _SAT in _SATs:
        samples[(_param, _SAT)] = _samples_ma[_param]
