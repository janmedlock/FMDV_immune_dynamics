...
class Sensitivity:
    def __init__(self, module, var, label, log=False):
        self.module = module
        self.var = var
        self.label = label
        self.log = log

    def load_extinction_time(self):
        return common.load_extinction_time(self.module.store_path)

    def plot_median(self, df, CI=0.5):
        levels = [CI / 2, 1 - CI / 2]
        with seaborn.axes_style('darkgrid'):
            ...
            for ((SAT, group), ax) in zip(df.groupby('SAT'), axes):
                times = group.groupby(self.var).time
                median = times.median()
    ...
    def plot_survival(self, df):
        (fig, axes) = matplotlib.pyplot.subplots(3, 1, sharex=True)
        for ((SAT, group), ax) in zip(df.groupby('SAT'), axes):
            for (idx, g) in group.groupby(self.var):
                survival = stats.get_survival(g, 'time', 'observed')
                ax.plot(survival.index, survival,
                        label=f'{self.var} {idx}',
    ...
    def plot_kde(self, df):
        with seaborn.axes_style('darkgrid'):
            ...
            for ((SAT, group), ax) in zip(df.groupby('SAT'), axes):
                subplotspec = ax.get_subplotspec()
                for (s, g) in group.groupby(self.var):
                    e = g.time.copy()
    ...
    def plot_kde_2d(self, df, save=True):
        rc = common.rc.copy()
        ...
        with matplotlib.pyplot.rc_context(rc=rc):
            ...
            for ((SAT, group_SAT), axes_col) in zip(grouper_SAT, axes.T):
                proportion_observed = get_proportion_observed(group_SAT,
                                                              self.var)
                density = get_density(group_SAT, self.var,
                                      extinction_time)
                ax = axes_col[0]
                cmap = get_cmap(SAT)
            ...
            for ax in fig.axes:
                ax.axvline(self.module.default,
            ...
            fig.tight_layout()
            if save:
                fig.savefig(self.module.store_path.with_suffix('.pdf'))
                fig.savefig(self.module.store_path.with_suffix('.png'),
                            dpi=300)
            return fig
