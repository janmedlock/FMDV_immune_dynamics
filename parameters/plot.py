...
ax[0].step(t, mortality.hazard(t), where = 'post', color = 'black')
ax[0].set_title('Death')
ax[0].set_xlabel('Age (years)')

ax[1].plot(t, birth.hazard(t, 0, 4 - t), color = 'black')
...
ax[0].plot(t, mortality.sf(t), color = 'black')
ax[0].set_title('Death')
ax[0].set_xlabel('Age (years)')
ax[0].xaxis.set_major_locator(ticker.MaxNLocator(2))

ax[1].plot(t, birth.sf(t, 0, 4), color = 'black')
ax[1].set_title('Birth')
ax[1].set_xlabel('Time (years)')
...
ax[2].step(t1, maternalImmunityWaning.sf(t1), where = 'post',
           color = 'black')
...
ax[3].step(t2, recovery.sf(t2 / 365.), where = 'post', color = 'black')
