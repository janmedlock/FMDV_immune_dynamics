...
def _copy_run_SATs(SAT, bscov, nruns, hdfstore_out):
...
def run_birth_seasonality(SAT, birth_seasonality_scaling, tmax, nruns,
                        hdfstore):
    ...
    if birth_seasonality_scaling == 1:
        _copy_run_SATs(SAT, bscov, nruns, hdfstore)
    else:
        logging_prefix = (
            ', '.join((
                f'SAT {SAT}',
...
if __name__ == '__main__':
    ...
    with h5.HDFStore(_filename) as store:
        for birth_scaling in birth_scalings:
            for SAT in (1, 2, 3):
                run_birth_seasonality(SAT, birth_scaling,
                                      tmax, nruns, store)
        store.repack()
