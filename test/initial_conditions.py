...
class Solver:
    ...
    @classmethod
    def get_A_YY(cls, rate_out_times_age_step):
        I = len(rate_out_times_age_step) + 1
        K = cls.get_k(I, 0)
        A = sparse.lil_matrix((K, K))
        k = cls.get_k(0, 0)
        A[k, k] = 1
        for i in range(1, I):
            k = cls.get_k(i, 0)
            A[k, k] = - 2
            for (j, r) in enumerate(rate_out_times_age_step[:i], 1):
                # k = cls.get_k(i, j)
                # k_1 = cls.get_k(i - 1, j - 1)
                [k, k_1] = cls.get_k([i, i - 1], [j, j - 1])
                A[k, [k, k_1]] = [- 2 - r, 2 - r]
        return A
    ...
    def get_row_E(self):
        ...
        for (i, h) in enumerate(hazard_infection, 1):
            k = self.get_k(i, 0)
            A_ES[k, [i, i - 1]] = h
            A_EL[k, [i, i - 1]] = h
        with numpy.errstate(divide='ignore'):
